...
if($_REQUEST['Key'] == '14583' && $_REQUEST['format'] == 'xml'){
	...
	$Account_ID = $_REQUEST['ACCOUNT'] = 17;
	...
	if ($GROUP=="ALL"){		
		$sql="select IMEI from DEVICE_REGISTER where Account_ID='$Account_ID' and IMEI in (".join(",",$IMEI_LIST).") order by Asset_No";
	}else{		
		$sql="select IMEI from DEVICE_REGISTER where Account_ID='$Account_ID' and IMEI in (select IMEI from DEVICE_GROUP where Group_Index='$GROUP') and IMEI in (and IMEI in (".$IMEI_LIST.")) order by Asset_No";
	}
	...
	$sql="select a.IMEI,b.Asset_No,b.Phone_No,b.Driver,a.Date_Stamp,a.Time_Stamp,a.Location_Name,a.Latitude,a.Longitude,a.Speed,b.GMT_Drift,b.SIM_No,c.Device_Type,b.Device_Status,b.Device_Index,b.Added_On from DEVICE_DATA_VIEW a,DEVICE_REGISTER b,DEVICE_TYPE c where a.IMEI=b.IMEI and b.IMEI in (".join(",",$IMEI_LIST).") and b.Device_Type_ID=c.Device_Type_ID order by b.Asset_No,a.Epoch_Time DESC,a.Speed DESC";
	$result = mysql_query($sql);
